...
def top_hoteis_bem_avaliados():
    return grafico_service.gerar_top_5_hoteis_mais_bem_avaliados()

@grafico_bp.route("hoteis/mal-avaliados", methods=['GET'])
def top_5_hoteis_mal_avaliados():
    return grafico_service.gerar_top_5_hoteis_mais_mal_avaliados()

@grafico_bp.route("/tipos-viagens", methods=['GET'])
def tipo_viajantes():
    cidade = request.args.get('cidade')
    data_inicio = request.args.get('data_inicio')  # Espera-se formato MM/YYYY
    data_fim = request.args.get('data_fim')        # Espera-se formato MM/YYYY
